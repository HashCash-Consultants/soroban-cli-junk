...
Intro: https://soroban.hcnet.org/docs
CLI Reference: https://github.com/hcnet/soroban-cli/tree/main/docs/soroban-cli-full-docs.md
...
Start a container running a Hcnet node, RPC, API, and friendbot (faucet).

soroban network start <NETWORK> [OPTIONS]

By default, when starting a testnet container, without any optional arguments, it will run the equivalent of the following docker command: docker run --rm -p 8000:8000 --name hcnet hcnet/quickstart:testing --testnet --enable-soroban-rpc
...
* `asset` — Utilities to deploy a Hcnet Asset Contract or get its id
...
Utilities to deploy a Hcnet Asset Contract or get its id
...
* `wrap` — Deploy a token contract to wrap an existing Hcnet classic asset for smart contract usage Deprecated, use `soroban contract deploy asset` instead
...
Deploy a token contract to wrap an existing Hcnet classic asset for smart contract usage Deprecated, use `soroban contract deploy asset` instead
